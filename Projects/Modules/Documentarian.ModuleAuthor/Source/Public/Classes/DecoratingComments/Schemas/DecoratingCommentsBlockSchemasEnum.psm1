...
class DecoratingCommentsBlockSchemasEnum : DecoratingCommentsBlockSchema {
    <#
        .SYNOPSIS
        Represents the schema for a comment decorating an enumeration.

        .DESCRIPTION
        This class represents the schema for a block comment that decorates the
        type definition of an enumeration in PowerShell code.

        It parses the following keywords from the decorating comment:

        - `Synopsis` - A brief description of the enumeration's purpose. The
          keyword must have a block of text after it.
        - `Description` - A full description of the enumeration, with details.
          The keyword must have a block of text after it.
        - `Example` - An example of using the enumeration. You can document
          multiple examples by specifying the keyword once for each example.
          The keyword must have a block of text after it.
        - `Label` - The description for one of the enumeration's values. You
          must specify the label of the value you're describing after the
          keyword on the same line, then a block of text after it. Specify the
          keyword once for each value.
        - `Notes` - Additional information about the enumeration. The keyword
          must have a block of text after it.
    #>
    ...
    static [DecoratingCommentsBlockKeyword[]] $Keywords = @(
        ...
        [DecoratingCommentsBlockKeywords]::Label
