...
class DecoratingCommentsBlockSchemasClass : DecoratingCommentsBlockSchema {
    <#
        .SYNOPSIS
        Represents the schema for a comment decorating a class.

        .DESCRIPTION
        This class represents the schema for a block comment that decorates the
        type definition of a class in PowerShell code.

        It parses the following keywords from the decorating comment:

        - `Synopsis` - A brief description of the class's purpose. The
          keyword must have a block of text after it.
        - `Description` - A full description of the class, with details.
          The keyword must have a block of text after it.
        - `Example` - An example of using the class. You can document
          multiple examples by specifying the keyword once for each example.
          The keyword must have a block of text after it.
        - `Notes` - Additional information about the class. The keyword
          must have a block of text after it.
        - `Method` - A brief description of a method the class implements. The
          keyword must have a block of text after it. Note that the synopsis
          for a method is separate from the synopsis for the method's
          overloads. Each overload may have its own synopsis and description.
    #>
