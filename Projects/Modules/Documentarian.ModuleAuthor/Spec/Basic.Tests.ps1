...
Describe 'Inspecting a source code for HelpInfo' -Tag Acceptance {
  Context 'when the source code defines a class' {
    Context 'without any comments' {
      BeforeAll {
        $ClassName = 'HelpInfoTestClassBare'
        $ClassFile = Resolve-Path -Path "$PSScriptRoot/Fixtures/Classes/Bare.psm1" |
      ...
      It 'the HelpInfo has no class-level documentation' {
    ...
    Context 'with non-parsing decorating comments' {
      BeforeAll {
        $ClassName = 'HelpInfoTestClassMinimal'
        $ClassFile = Resolve-Path -Path "$PSScriptRoot/Fixtures/Classes/Minimal.psm1" |
          Select-Object -ExpandProperty Path
